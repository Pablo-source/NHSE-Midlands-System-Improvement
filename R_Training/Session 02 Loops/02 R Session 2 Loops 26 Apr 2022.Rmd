...
output:
  html_document:
    df_print: paged
---
This session describes some control structures in R such as loops, exploring the different types of loops and its components. 
...
print(x)
```
As we saw in the first session, we can retrieve some properties from our vector. Also by using the c(:) operator, we ensure we run through all the elements from the left side of ":" until the number on the right side of the ":". 

This is a way of ensuring we account for all the different elements in a vector
...
```
For learning purposes, we have created the above for loop. It has a small bug: 
...
Once the first iteration is complete, the for loops back to the beginning and replaces i with the next value in our 1:45 sequence (2 in this case).

The process is repeated until the loop reaches last value of the sequence.

This is very powerful as we can use it to iterate over different objects in R. We can use it to iterate over a vector to retrieve each of its elements, one at a time.
...
Let's say you want to start looping through a vector or data frame, not on the first element (because if might have the column names or row labels), and we want instead to start looping from the second element onwards.  
...
####  **5.1 Indexing elements in a Data frame**
...
####  **5.2 Idexing rows and columns in a data frame** 
...
We select all rows by leaving *blank* the first portion of the two square brackets, meaning that we can loop through *all" elements in that row. 
...
Now applying this same principle, we can obtain the number of admissions by trust by entering the data frame name with an index into the loop.
...
str(Mydata)
```
As we can see from the data set above, the months are read by R as Factors, so they are not in alphabetical order. We can fix this by defining month values based on its variable values, using the factor() function: 
...
We use par() function to setup graphical parameters. In this instance, we use it with mfrow(), so specify a matrix with a vector defined by c(number of rows, number of cols) to populate it with our plots. mfrow() is a vector object a vector of the form c(nr, nc) as array on the device by columns (mfcol), or rows (mfrow), respectively.
...
1. **par()** function
  ...
  - For this particular example, we want to create a report with **three indicators** for each Trust. So we use par(mfrow=c(1,3)) to create the matrix that will be populated with three different plots.
...
Below we can see our final script, we have changed the content of the for loop to go from 2 to NORG instead of 1 to NORG, can anyone tell me why?
...
We obtain this by adding the following two lines **before** the par() statement inside the loop
...
This would be the final script for the final output, it would include all the new features we just described
...
- This is just a small example on how to create a report using R to include three set of indicators and output one PDF output for each Trust 
